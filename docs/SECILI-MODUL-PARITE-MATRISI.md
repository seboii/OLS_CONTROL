...
| Silme | — | — | `DELETE /load_transfer` — **KAYNAKTA KIRIK** (yanlış `PaymentType` sınıfı) | `DELETE /api/v1/load_transfer` `Delete` | `{message}` | — | `load_management`(delete) | — | H | **[FAZ 5 güvenlik taramasında doğrulandı]** olsnew'de çalışır hale getirilmiş VE gerçek `[RequiresPermission(Delete,"load_management")]` uygulanıyor — olsold'un kırık/yanlış-yetkili hali kopyalanmadı |
| Paket/kalem silme | — | — | `DELETE /load_transfer/load_transfer_package`, `/load_transfer_invoice_item` — kaynakta yetkisiz/yorumda | aynı yollar | `{message}` | `load_transfer_packages`,`...invoice_items` | `load_management`(delete) | — | H | **[FAZ 5'te doğrulandı]** Her iki uçta da gerçek `[RequiresPermission(Delete,"load_management")]` var — kaynaktaki "yetki yok" durumu kopyalanmadı |
| Fatura kalemi CRUD | — | `InvoiceFormInvoiceItems.vue` üzerinden dolaylı | `/load_transfer_invoice_item[...]` — **save/update KIRIK** (olmayan `name`/`siber_id` kolonuna yazıyor) | `LoadTransferInvoiceItemController` | `{data,message}` | `load_transfer_invoice_items` | `load_management`(tam CRUD) | — | H | **[FAZ 5'te doğrulandı]** olsnew doğru kolonlara yazıyor (`insert_name`,`modulkalemid`) VE GET/POST/PUT/DELETE'in dördü de `[RequiresPermission(...,"load_management")]` ile korunuyor — önceki "yalnızca DELETE korumalı" notu STALE, boşluk kapatılmış |
| Yük hareketleri | — | `LoadFormMovements.vue`(342) | `/load_transfer_movement[...]` — kaynakta yetki YOK | `LoadTransferMovementController` | `{status,message,data,deleted_movements}` | `load_transfer_movements` | `load_management`(tam CRUD) | — | H | **[FAZ 5'te doğrulandı]** Tüm 5 uç (`GET`/`GET id`/`POST`/`POST update`/`DELETE`) `[RequiresPermission(...,"load_management")]` ile korunuyor — önceki "yetki YOK" notu STALE |
...
| Ekran/işlem | Eski FE | Yeni FE | Laravel | .NET | İstek/Response | Tablolar | Yetki | Bağımlılık | Durum | Karar |
|---|---|---|---|---|---|---|---|---|---|---|
| Liste+detay | — | `expedition/list.vue`,`form.vue`→`ExpeditionTable.vue`(51),`ExpeditionFormDrawer.vue`(383) | `GET /expedition`,`/expedition/{id}` | `GET /api/v1/expedition`(+`/{id}`) | `{data,message}` | `expeditions` | yok (kaynak) / `expedition_management`(read) | Car,City,WorkType,Department | H | Birebir. **[FAZ 5'te düzeltildi]** Yetki sütunu önceden yanlışlıkla `load_management` diyordu — gerçek slug `expedition_management` (bkz. aşağıdaki satırlarla tutarlı) |
| Oluşturma | aynı | aynı | `POST /expedition` `save` | `POST /api/v1/expedition` `Save` | Siber sefer/pozisyon numaralama | `expeditions`+Siber `skn_sefer/skn_pozisyon` | `expedition_management`(create) | RomorkType(Car uyumu, BR-006) | H | **[FAZ 5 güvenlik taramasında doğrulandı]** Özel `expedition_management` slug'ı netleşmiş — `load_management` ile paylaşılmıyor. BR-006 (araç/römork tipi uyumu) birebir |
| Güncelleme | aynı | aynı | `PUT /expedition` | `PUT /api/v1/expedition` `Update` | `expedition_status_id==8` özel alan seti | aynı | `expedition_management`(update) | ExpeditionStatus | H | Birebir |
| Silme | aynı | aynı | `DELETE /expedition` — **kaynakta `case_type_management` leak** | `DELETE /api/v1/expedition` `Delete` | `{message}` | — | `expedition_management`(delete) | — | H | **[FAZ 5'te doğrulandı]** Kaynağın leak'i kopyalanmadı — özel `expedition_management`(delete) uygulanıyor |
| Sefer hareketleri | — | `ExpeditionFormMovements.vue`(325) | `/expedition/{id}/movements[...]` — **kaynakta id parametresi kullanılmıyor (body'den okunuyor) — BUG** | `MovementService` (`LoadTransferController.cs` içindeki `ExpeditionController`) | `{status,message,data,deleted_movements}` | `expedition_movements` | `expedition_management`(tam CRUD) | Load zinciri (BR-010) | H | **[FAZ 5'te doğrulandı]** Yetki: tüm 3 uç (`GET`/`POST`/`DELETE movements`) korunuyor. Kaynağın "id gövdeden okunuyor" davranışı BİLİNÇLİ korundu (kod yorumunda açık): rota `{id}` alır ama sefer kimliği önce gövdeden okunur, gövde boşsa rota parametresine düşülür — kaynakla birebir + daha sağlam. BR-010 için hâlâ ayrı test yok |
| Yük-sefer bağlama | — | `ExpeditionLoad.vue`(241) | `/expedition_load_mapping[...]` | `ExpeditionLoadMappingController` | `{data,message}`/`{data,total_expedition_values,message}` | `expedition_load_mappings`+Siber `skn_yukaktarma` | `expedition_management`(tam CRUD) | — | H | **[FAZ 5'te doğrulandı]** Tüm 5 uç `[RequiresPermission(...,"expedition_management")]` ile korunuyor — önceki "sıfır CRUD yetkisi" notu STALE. BR-007 (aynı yük tekrar bağlanamaz) ve iki-DB transaction korunuyor |
...
| Silme | — | — | `DELETE /car` — **`car_management` slug'ı seed'de YOK, kod bunu bildiği yorumla itiraf ediyor** | `DELETE /api/v1/car` | `{message}` | — | `car_management`(delete) | — | H | **[FAZ 5 güvenlik taramasında doğrulandı]** `car_management` artık `DbSeeder.cs`'de gerçekten seed ediliyor ("Araç Yönetimi") ve `CarController`'ın TÜM uçları (`[RequiresPermission]`) buna bağlı — bariz güvenlik boşluğu kapatıldı, "fiilen herkese açık" durumu artık YOK |
| CarType/CarOwner/CarStatus/RomorkType CRUD | — | `FeatureModals/` içindeki hızlı-ekle modalleri (Offer/Load/Expedition/Car formlarına gömülü, **ayrı admin ekranı değil**) | 4 ayrı controller — **kaynakta save/update/delete TAMAMEN KIRIK** (yanlış donör sınıf import edilmemiş → fatal) | `LookupControllerBase<T>` generic altyapı üzerinden — olsnew'de ÇALIŞIYOR | `{data,message}` | `car_types`,`car_owners`,`car_status_types`,`romork_types` | `case_type_management`(paylaşımlı, kaynaktan kasıtlı korunmuş) | — | H | olsnew'in çalışan generic altyapısı taşındı — olsold'un "sadece okuma çalışır" kırıklığı kopyalanmadı. Paylaşımlı yetki slug'ı davranış parites için bilinçli korunuyor. **[FAZ 5'te doğrulandı]** `LookupControllerBase`'in her 5 ucu da (attribute yerine gövde-içi `HasPermissionAsync`/`PermissionSlug` deseniyle) gerçekten yetki kontrolü yapıyor — teslim raporu §1 Kritik yön değişikliği #11'de not edildi |
...
| Yetki atama (sayfa×CRUD) | — | `UserRole.vue`(139, drawer içi tab) | `GET/PUT /role` | `GET/PUT /api/v1/role` `RoleController` | `{id,stats:{...}}` / `{result:'success'}` (**kaynakta yanlış giden çağrıda da her zaman 'success' dönüyor — BUG**) | `user_permission_pages`,`user_permissions` | own-record veya `role_management` | — | H | **[Doğrulandı]** olsnew'de kendi-kaydı-okuma güvenli biçimde ayrılmış VE `Update` yetkisizken kaynağın "her zaman success" hatası kopyalanmadı — kod yorumunda açık ("Burada gerçekten 403 veriyoruz"), gerçek `403 Forbidden` dönüyor; `RoleEndpointContractTests.cs` ile kilitli. Ayrıca "Tümünü Seç" (permission_page_id'siz, user_id'li toplu güncelleme) bu güncellemede Kullanıcılar formuna bağlandı — bkz. TESLIM-RAPORU.md §1 Kritik yön değişikliği #9 |
...
| Kullanıcı hedefi (Hedef sekmesi) | — | `UserTarget.vue`(223, drawer içi tab) | `/user_goal[...]` | `UserGoalController` (`/api/v1/user_goal`) | `{data,message}` | `user_goals` | `user_management`(tam CRUD) | — | H | **[TAMAMLANDI — bkz. TESLIM-RAPORU.md §1 Kritik yön değişikliği #9]** Bu satırın "istisnai kapsam-içi bağımlılık" kararı, projenin özet belgelerinden biriyle (DependencyInjection.cs'in eski yorumu) çelişince kullanıcıya soruldu; eklenmesi onaylandı. Backend+frontend tam CRUD + kaynağın tarih-aralığı çakışma kuralı; kaynakta `delete()`'in yorum satırındaki (fiilen herkese açık) yetki kontrolü yerine gerçek `user_management` yetkisi uygulanıyor. `UserGoalTests.cs` (6 test) ile kilitli |
...
|---|---|---|---|---|---|---|---|---|---|
| Dış form gönderimi (public) | (harici site) | (harici site, uygulama dışı) | `POST /api/website/contact/form` — auth YOK (kasıtlı) | `POST /api/website/contact/form` `[AllowAnonymous]` `[EnableRateLimiting("public-form")]` | `{success,message,data}` 201 | `website_contact_forms` | yok (kasıtlı) | H | Birebir + **[Doğrulandı]** rate limit eklendi (5/dk, Program.cs) — spam koruması aktif |
| Liste | — | `pages/website/contact/forms.vue`→`FormsTable.vue`(96) | `GET /api/website/contact/form` — **KAYNAKTA TAMAMEN ANONİM (SEC-003)** | `GET /api/website/contact/form` | `{success,data:sayfalı}` | aynı | `support_request_management`(read) | H | **[TAMAMLANDI]** Kaynağın anonim-okuma açığı kopyalanmadı — gerçek `support_request_management` sayfa yetkisi + CRUD bayrakları uygulanıyor. **[Bu güncellemede eklendi]** kaynağın `FormsTable.vue`'sindeki arama kutusu görsel-ama-işlevsizdi (`index()` hiçbir istek parametresi okumuyordu) — bilinçli olarak gerçek arama eklendi (ad/soyad/e-posta/telefon/mesaj). `ContactFormTests.cs` (5 test) ile kilitli |
| Detay (okundu işaretleme yan etkisi) | — | `FormDetailDrawer.vue`(152) | `GET /api/website/contact/form/{id}` — anonim, GET yan etkili (`is_read=true`) | aynı yol, `support_request_management`(read) | `{success,data}`/404 | aynı | `support_request_management`(read) | H | Yan etki (GET'in okundu işaretlemesi) davranış parites için korunuyor; yetki eklendi. **[Bu güncellemede eklendi]** detay panelinde eksik olan gönderim tarihi alanı eklendi (kaynakta bu alan kopyala-yapıştır hatasıyla "Telefon Numarası" diye yanlış etiketlenmişti — veri taşındı, hata değil) |
| Yanıtlandı işaretleme | — | aynı drawer | `PATCH .../answered` — anonim | aynı, `support_request_management`(update) | `{success,message,data}` | aynı | `support_request_management`(update) | H | Yetki eklendi |
