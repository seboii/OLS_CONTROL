    ...
    <div>
      <div class="mb-4 lg:mb-8 flex justify-between items-center gap-6">
        <div>
          <div class="mb-4 lg:mb-8">
            <RouterLink to="/panel/expedition">
          ...
          <h2 class="text-gray-800 font-medium text-4xl tracking-tight">
            {{ route.params.id ? "Sefer Düzenleme" : "Yeni Sefer Ekle" }}
          </h2>
          <div class="text-gray-500 text-sm mt-1">
            {{
              route.params.id
                ? "Sefer bilgilerini düzenlemek için gerekli alanları eksiksiz doldurunuz."
                : "Yeni sefer eklemek için gerekli alanları eksiksiz doldurunuz."
            }}
          </div>
        </div>
      </div>
      <div>
        <Tabs value="0" scrollable lazy>
          <TabList v-if="form_data.id" class="text-sm!">
          ...
          <TabPanels class="py-6! px-0!">
                ...
                <template #content>
                  ...
                  <div v-else>
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-y-6 gap-x-4">
                      <div v-if="route.params.id" class="lg:col-span-12">
                      ...
                      <hr v-if="route.params.id" class="lg:col-span-12" />
                      <div :class="route.params.id ? 'lg:col-span-12' : 'lg:col-span-6'">
                      ...
                      <div v-if="route.params.id" class="lg:col-span-6">
                        <FloatLabel variant="on">
            ...
            <TabPanel value="1">
              <ExpeditionLoad :expedition-id="route.params.id" />
            </TabPanel>
          </TabPanels>
        </Tabs>
      </div>
...
onMounted(async () => {
  if (route.params.id) {
    ...
    const res = await DataStore.GET_EXPEDITION(route.params.id);
    if (res) {
      const data = res.data;
    ...
    if (route.query.work_type_id) {
      const res = await DataStore.GET_WORK_TYPE(route.query.work_type_id);
      if (res) {
