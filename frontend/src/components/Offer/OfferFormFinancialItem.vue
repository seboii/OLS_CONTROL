    ...
    <div class="rounded-xl p-5 lg:py-5 lg:px-8 border bg-white shadow-xs flex items-start lg:items-end justify-between max-lg:flex-col gap-4">
      <div class="w-full">
        ...
        <div class="flex flex-wrap gap-2 max-lg:flex-col">
          ...
          <div class="text-xs text-gray-500 py-2 px-3 bg-gray-50 rounded-lg">
            Adeti : <span class="font-medium">{{ parseFloat(data.quantity) }}</span>
          </div>
          <div class="text-xs text-gray-500 py-2 px-3 bg-gray-50 rounded-lg">
            Taşıma Tipi : <span class="font-medium">{{ data.transport_type_id?.name }}</span>
          </div>
          <div class="text-xs text-gray-500 py-2 px-3 bg-gray-50 rounded-lg">
            Toplam Tutar : <span class="font-medium">{{ data.total_price }}</span>
          </div>
        </div>
      </div>
      <div class="flex items-center justify-end gap-2">
        <Button @click="item_edit_dialog_visible = true" size="small" outlined :label="editable ? 'Düzenle' : 'Görüntüle'" />
        <Button v-if="editable" @click="emits('delete', data)" severity="danger" outlined size="small" label="Sil" class="px-6!" />
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 my-10">
        ...
        <div v-if="false">
          <FloatLabel variant="on">
            <SelectAjax v-model="data.item_type_id" :api="`/api/v1/item_type`" optionLabel="name" class="w-full" filter :disabled="!editable" />
        ...
        <FloatLabel variant="on">
          <SelectAjax
            ...
            :disabled="!data.buysell || !editable"
            optionLabel="name"
            class="w-full"
            filter
          >
        ...
        <FloatLabel class="w-full" variant="on">
          <InputText @value-change="() => calculateSingleTotalPrice(data)" v-model="data.quantity" v-keyfilter.int fluid :disabled="!editable" />
          <label> Adet </label>
        </FloatLabel>
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            @value-change="() => calculateSingleTotalPrice(data)"
            v-model="data.net_price"
            :maxFractionDigits="2"
            v-keyfilter.money
            fluid
            :disabled="!editable"
        ...
        <FloatLabel v-if="false" class="w-full" variant="on">
          <InputNumber v-model="data.tax_price" :maxFractionDigits="2" v-keyfilter.money fluid :disabled="!editable" />
          <label> Vergi Tutarı </label>
        </FloatLabel>
        <FloatLabel class="w-full" variant="on">
          <InputNumber v-model="data.total_price" :maxFractionDigits="2" v-keyfilter.money fluid :disabled="!editable" />
          <label> Toplam Tutar </label>
        </FloatLabel>
        <div>
          <FloatLabel variant="on">
            <Select v-model="data.transport_type_id" :options="define_datas.offer.transport_type" optionLabel="name" fluid :disabled="!editable" />
        ...
        <div v-if="false">
          <FloatLabel variant="on">
            <Select v-model="data.status" :options="financial_item_status_type" optionLabel="name" fluid :disabled="!editable" />
...
const currency_select_ref = ref(null);

console.log(currency_select_ref);
