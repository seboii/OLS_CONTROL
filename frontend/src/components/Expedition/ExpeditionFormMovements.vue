...
watch(
  ...
  (newVal) => {
    if (newVal) {
      movementForm.value.expedition_id = newVal;
      fetchMovements();
    }
  }
);
...
const fetchMovements = async () => {
  if (!props.expeditionId) return;

  loading.value = true;
  try {
    const response = await dataStore.GET_EXPEDITION_MOVEMENTS(props.expeditionId);
    movements.value = response?.data || [];
    deletedMovements.value = response?.deleted_movements || [];
    console.log(deletedMovements.value, response);
  } catch (error) {
...
const editMovement = (movement) => {
  ...
  movementForm.value = {
    id: movement.id,
    expedition_id: movement.expedition_id,
...
const saveMovement = async () => {
  ...
  try {
    ...
    console.log(props.expeditionId);

    if (movementForm.value.id) {
      form_data.append("id", movementForm.value.id);
    }
    form_data.append("expedition_id", movementForm.value.expedition_id);
    ...
    if (editMode.value) {
      await dataStore.UPDATE_EXPEDITION_MOVEMENT({ id: movementForm.value.id, data: form_data });
      toast.success("Hareket başarıyla güncellendi");
    } else {
      await dataStore.CREATE_EXPEDITION_MOVEMENT({ id: movementForm.value.expedition_id, data: form_data });
...
const deleteMovement = async (movement) => {
  try {
    await dataStore.DELETE_EXPEDITION_MOVEMENT({ expedition_id: movement.expedition_id, movement_id: movement.id });
...
onMounted(() => {
  if (props.expeditionId) {
    movementForm.value.expedition_id = props.expeditionId;
    fetchMovements();
  }
});
</script>
