<template>
  <div>
    <Dialog
      v-model:visible="FeatureStore.load_financial_item_types_modal_status"
      modal
      header="Yük Finansal Ürün Tipleri"
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Yük Finansal Ürün Tipi Düzenle' : 'Yük Finansal Ürün Tipi Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/item_type",
  deleteUrl: "/api/v1/item_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_LOAD_FINANCIAL_ITEM_TYPE({ data: type_data.data });
    toast.success("Yük finansal ürün tipi başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_LOAD_FINANCIAL_ITEM_TYPE({ data: type_data.data });
    toast.success("Yük finansal ürün tipi başarıyla oluşturuldu.");
