<template>
  <div>
    <Dialog v-model:visible="FeatureStore.account_types_modal_status" modal header="Hesap Tipleri" @show="onShowModal" class="w-full lg:w-[600px]">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Hesap Tipi Düzenle' : 'Hesap Tipi Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/account_type",
  deleteUrl: "/api/v1/account_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_ACCOUNT_TYPE({ data: type_data.data });
    toast.success("Hesap tipi başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_ACCOUNT_TYPE({ data: type_data.data });
    toast.success("Hesap tipi başarıyla oluşturuldu.");
