<template>
  <div>
    <Dialog v-model:visible="FeatureStore.load_product_types_modal_status" modal header="Yük Ürün Tipleri" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Yük Ürün Tipi Düzenle' : 'Yük Ürün Tipi Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/product_type",
  deleteUrl: "/api/v1/product_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_LOAD_PRODUCT_TYPE({ data: type_data.data });
    toast.success("Yük ürün tipi başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_LOAD_PRODUCT_TYPE({ data: type_data.data });
    toast.success("Yük ürün tipi başarıyla oluşturuldu.");
