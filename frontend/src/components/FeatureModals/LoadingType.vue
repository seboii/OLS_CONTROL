<template>
  <div>
    <Dialog v-model:visible="FeatureStore.loading_types_modal_status" modal header="Yükleme Durumları" @show="onShowModal" class="w-full lg:w-[600px]">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Yükleme Tipi Düzenle' : 'Yükleme Tipi Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/loading_type",
  deleteUrl: "/api/v1/loading_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_LOADING_TYPE({ data: type_data.data });
    toast.success("Yükleme Tipi başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_LOADING_TYPE({ data: type_data.data });
    toast.success("Yükleme Tipi başarıyla oluşturuldu.");
