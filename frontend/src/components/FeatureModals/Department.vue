<template>
  <div>
    <Dialog v-model:visible="FeatureStore.departments_modal_status" modal header="Departmanlar" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog v-model:visible="type_data.modal_status" modal :header="type_data.data.id ? 'Departman Düzenle' : 'Departman Oluştur'" class="w-full lg:w-[500px]">
      <div class="my-8">
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/department",
  deleteUrl: "/api/v1/department",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_DEPARTMENT({ data: type_data.data });
    toast.success("Departman başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_DEPARTMENT({ data: type_data.data });
