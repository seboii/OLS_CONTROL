<template>
  <div>
    <Dialog v-model:visible="FeatureStore.account_group_code_modal_status" modal header="Hesap Grup Kodları" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Hesap Grup Kodu Düzenle' : 'Hesap Grup Kodu Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/account-group-code",
  deleteUrl: "/api/v1/account-group-code",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_ACCOUNT_GROUP_CODE({ data: type_data.data });
    toast.success("Hesap Grup Kodu başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_ACCOUNT_GROUP_CODE({ data: type_data.data });
    toast.success("Hesap Grup Kodu başarıyla oluşturuldu.");
