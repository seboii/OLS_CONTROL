<template>
  <div>
    <Dialog v-model:visible="FeatureStore.movement_types_modal_status" modal header="Hareket Durumları" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Hareket Durumu Düzenle' : 'Hareket Durumu Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/movement_type",
  deleteUrl: "/api/v1/movement_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_MOVEMENT_TYPE({ data: type_data.data });
    toast.success("Hareket durumu başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_MOVEMENT_TYPE({ data: type_data.data });
    toast.success("Hareket durumu başarıyla oluşturuldu.");
