<template>
  <div>
    <Dialog v-model:visible="FeatureStore.work_types_modal_status" modal header="İşlem Tipleri" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog v-model:visible="type_data.modal_status" modal :header="type_data.data.id ? 'İş Türü Düzenle' : 'İş Türü Oluştur'" class="w-full lg:w-[500px]">
      <div class="my-8">
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/work_type",
  deleteUrl: "/api/v1/work_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_WORK_TYPE({ data: type_data.data });
    toast.success("İş Türü başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_WORK_TYPE({ data: type_data.data });
    toast.success("İş Türü başarıyla oluşturuldu.");
