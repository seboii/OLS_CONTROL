<template>
  <div>
    <Dialog v-model:visible="FeatureStore.status_types_modal_status" modal header="Yük Durumları" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Yük Durumu Düzenle' : 'Yük Durumu Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/status_type",
  deleteUrl: "/api/v1/status_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_STATUS_TYPE({ data: type_data.data });
    toast.success("Yük durumu başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_STATUS_TYPE({ data: type_data.data });
    toast.success("Yük durumu başarıyla oluşturuldu.");
