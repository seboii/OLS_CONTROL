<template>
  <div>
    <Dialog v-model:visible="FeatureStore.payment_types_modal_status" modal header="Ödeme Tipleri" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog
      ...
      modal
      :header="type_data.data.id ? 'Ödeme Şekli Düzenle' : 'Ödeme Şekli Oluştur'"
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/payment_type",
  deleteUrl: "/api/v1/payment_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_PAYMENT_TYPE({ data: type_data.data });
    toast.success("Ödeme Şekli başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_PAYMENT_TYPE({ data: type_data.data });
    toast.success("Ödeme Şekli başarıyla oluşturuldu.");
