<template>
  <div>
    <Dialog v-model:visible="FeatureStore.vessel_types_modal_status" modal header="Kap Tipleri" class="w-full lg:w-[600px]" @show="showModal">
    ...
    <Dialog v-model:visible="type_data.modal_status" modal :header="type_data.data.id ? 'Kap Tipi Düzenle' : 'Kap Tipi Oluştur'" class="w-full lg:w-[500px]">
      <div class="my-8">
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/case_type",
  deleteUrl: "/api/v1/case_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_VESSEL_TYPE({ data: type_data.data });
    toast.success("Kap tipi başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_VESSEL_TYPE({ data: type_data.data });
    toast.success("Kap tipi başarıyla oluşturuldu.");
