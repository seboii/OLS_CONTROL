<template>
  <div>
    <Dialog v-model:visible="FeatureStore.transport_types_modal_status" modal header="Taşıma Tipleri" @show="onShowModal" class="w-full lg:w-[600px]">
    ...
    <Dialog v-model:visible="type_data.modal_status" modal :header="type_data.data.id ? 'Yük Türü Düzenle' : 'Yük Türü Oluştur'" class="w-full lg:w-[500px]">
      <div class="my-8">
...
const TypeDatatable = useDatatable({
  getUrl: "/api/v1/transport_type",
  deleteUrl: "/api/v1/transport_type",
  perPage: 5,
});
...
const handleSubmit = async () => {
  ...
  if (type_data.data.id) {
    await DataStore.UPDATE_TRANSPORT_TYPE({ data: type_data.data });
    toast.success("Yük Türü başarıyla güncellendi.");
    TypeDatatable.refresh();
  } else {
    await DataStore.CREATE_TRANSPORT_TYPE({ data: type_data.data });
    toast.success("Yük Türü başarıyla oluşturuldu.");
